...
function validate() {
  ...
  if (!form.value.firstnames.trim()) {
    errors.value.firstnames = 'Los nombres son obligatorios.'
  }
...
async function handleSubmit() {
  ...
  try {
    const payload = { ...form.value, groupId }
    const response = await window.electronAPI.student.create(payload)
    if (!response.success) {
      submitError.value = response.error || 'Error creando grupo'
      return
    }
    // Redirigir automáticamente al dashboard del nuevo grupo
    router.replace({ name: 'GrupoDashboard', params: { id: groupId } })
  } catch (err) {
