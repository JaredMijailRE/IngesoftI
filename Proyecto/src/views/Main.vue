      ...
      <div class="container mx-auto px-4 py-3">
        <div class="text-left mb-12">
          <h1 class="text-4xl md:text-6xl font-bold text-gray-700 my-4">
            Welcome, <span class="text-sportu-600">{{ usuario.username }}</span
            >!
          </h1>
        </div>
        ...
        <div
          class="grid md:grid-cols-2 lg:grid-cols-2 gap-8 mb-8 mx-4"
        ...
        >
          <div class="card p-6 shadow-lg transition-shadow">
            ...
            <div
              v-else
              class="text-gray-600 flex flex-col items-center justify-center h-48"
            >
              <p class="text-gray-500">No tienes grupos asignados</p>
              <button
                class="bg-white hover:bg-sportu-100 text-gray-400 font-bold px-2.5 py-1 rounded-full"
                @click="toCrearGrupo()"
              >
                +
              </button>
          ...
          <div class="card p-6 shadow-lg transition-shadow">
            ...
            <div v-if="usuario.eventos.length != 0">
              <ul>
                <li
                  v-for="evento in usuario.eventos.slice(0, 3)"
                  :key="evento.id"
                  class="text-gray-600 mb-2"
                >
                  <button
                    class="btn-primary bg-sportu-500 w-full px-5 py-3.5 text-lg hover:bg-sportu-700"
                    @click="toEvento(evento.id)"
                  >
                    <div class="column">
                      <p>{{ evento.nombre }}</p>
                      <p class="text-sportu-100 text-xs">{{ evento.fecha }}</p>
                    </div>
                  </button>
                </li>
              </ul>
